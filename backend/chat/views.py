...
def pet_chat_api(request, pet_id):
    ...
    if request.method == "GET":
        ...
        # If there is no history yet, inject the opening message as a synthetic
        # first message so the owner sees it immediately without sending anything.
        # It is marked with opening=True so the frontend can style it differently
        # if desired, and it is NOT saved to the DB at this point.
        if not messages_out:
            opening = None
            try:
